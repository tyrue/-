...
class Interpreter
	...
	def update
		...
		loop do
			...
			# If map is different than event startup time, change event ID to 0
			@event_id = 0 if $game_map.map_id != @map_id
			# If a child interpreter exists
			if @child_interpreter != nil
				# Update child interpreter
				@child_interpreter.update
				# If child interpreter is finished running; delete child interpreter
			...
			# If list of event commands is empty and main map event; 
			# Set up starting event
			setup_starting_event if @main and @list == nil
