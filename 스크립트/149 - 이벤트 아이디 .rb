...
if SDK.state('Event Text Display') == true
	...
	class Game_Character
		...
		Item_Color = Color.new(225, 255, 255)
	...
	class Sprite_Character < RPG::Sprite
		...
		def update_display_text
			return dispose_display_text unless @character.text_display
			
			if @_text_display.nil? || @_old_id != @character.text_display
				@_old_id = @character.text_display
				create_display_sprite(@character.text_display) 
			end
			return unless @_text_display
		...
		def toggle_id
			return unless @_text_display
			return if @_text_display.disposed?
			
			@_text_display.visible = $game_variables[11] != 0
		end
