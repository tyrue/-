class MrMo_ABS
	...
	ITEM_DROP_DATA[80] = [[-1], [2, 33, 1, 15]] # 청명도깨비 : 도깨비부적
	...
	ITEM_DROP_DATA[83] = [[-1], [0, 61, 1, 60]] # 불구미호 : 수정의조각
	...
	ITEM_DROP_DATA[111] = [[2], [0, 70, 3, 60], [1, 125, 1, 20], [1, 129, 1, 15], [2, 44, 1, 10], [1, 130, 1, 10]] # 산적왕 : 빨간시약, 일월대도, 도깨비방망이, 황혼의활복, 산적왕의칼
	...
	ITEM_DROP_DATA[121] = [[-1], [0, 122, 1, 2], [0, 30, 1, 40]] # 새끼용 : 은나무가지, 진호박
	...
	ITEM_DROP_DATA[132] = [[1], [0, 123, 1, 70], [0, 122, 2, 40]] # 건룡 : 건룡의어금니, 은나무가지
	ITEM_DROP_DATA[133] = [[1], [0, 124, 1, 70], [0, 122, 2, 40]] # 감룡 : 감룡의어금니, 은나무가지
	ITEM_DROP_DATA[134] = [[1], [0, 125, 1, 70], [0, 122, 2, 40]] # 진룡 : 진룡의어금니, 은나무가지
	...
	ITEM_DROP_DATA[191] = [[-1], [0, 155, 1, 50]] # 유성지 : 유성지의보패
	ITEM_DROP_DATA[192] = [[-1], [0, 151, 1, 50]] # 해골왕 : 해골왕의뼈
	ITEM_DROP_DATA[193] = [[-1], [0, 154, 1, 60]] # 파괴왕 : 순수의강철
	...
	ITEM_DROP_DATA[204] = [[1], [0, 169, 1, 100], [0, 167, 1, 100]] # 인묘 : 녹색구슬조각, 갈색구슬조각
	...
	ITEM_DROP_DATA[213] = [[-1], [0, 190, 1, 10]] # 청악어 : 악어의피
	ITEM_DROP_DATA[214] = [[-1], [0, 190, 1, 10]] # 자악어 : 악어의피
	...
	ITEM_DROP_DATA[99] = [[1], [0, 60, 1, 60], [0, 201, 1, 40], [0, 202, 1, 10], [0, 203, 1, 10], [0, 204, 1, 10]] # 귀웅 : 쇠조각, 대지의토템, 바람의토템, 번개의토템, 화염의토템
	
	ITEM_DROP_DATA[241] = [[-1], [0, 208, 1, 40]] # 인면벌 : 인면벌의알
	ITEM_DROP_DATA[242] = [[-1], [0, 60, 1, 20]] # 문비 : 쇠조각
	
	# 난파선
	ITEM_DROP_DATA[243] = [[-1], [0, 209, 1, 10]] # 최가 : 기름자루
	ITEM_DROP_DATA[244] = [[-1], [0, 209, 1, 10]] # 백보 : 기름자루
	ITEM_DROP_DATA[245] = [[1], [0, 209, 1, 10], [0, 211, 1, 20]] # 엽춘 : 기름자루, 향료가루
	ITEM_DROP_DATA[246] = [[2], [0, 209, 1, 60], [0, 211, 1, 60], [0, 201, 1, 10], [0, 202, 1, 40], [0, 203, 1, 10], [0, 204, 1, 10], [0, 213, 1, 10]] # 선장망령 : 기름자루, 향료가루, 대지의토템, 바람의토템, 번개의토템, 화염의토템, 정령의결정
	
	# 가릉도 숲지대
	ITEM_DROP_DATA[247] = [[-1], [0, 161, 1, 30]] # 사마귀 : 희귀호박
	ITEM_DROP_DATA[248] = [[1], [0, 161, 1, 30], [0, 214, 1, 10]] # 독화 : 희귀호박, 독니
	ITEM_DROP_DATA[249] = [[2], [0, 201, 1, 10], [0, 202, 1, 10], [0, 203, 1, 40], [0, 204, 1, 10], [0, 214, 3, 40], [0, 218, 1, 20]] # 야월진랑 : 대지의토템, 바람의토템, 번개의토템, 화염의토템, 독니, 무소의뿔
	
	# 폭염도 화산굴
	ITEM_DROP_DATA[250] = [[1], [0, 162, 1, 30], [0, 215, 1, 5]] # 철보장 : 희귀진호박, 금조각
	ITEM_DROP_DATA[251] = [[1], [0, 162, 1, 30], [0, 216, 1, 5]] # 철거인 : 희귀진호박, 반짝이는돌
