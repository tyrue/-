class MrMo_ABS
	...
	# 극지방
	ITEM_DROP_DATA[105] = [[-1], [0, 103, 20, 50], [0, 116, 1, 30]] # 에메랄드인형 : 얼음, 황금호박
	ITEM_DROP_DATA[109] = [[-1], [0, 103, 2, 40]] # 눈괴물 : 얼음
	ITEM_DROP_DATA[110] = [[2], [0, 103, 2, 40], [0, 101, 2, 20], [0, 58, 1, 2]] # 북극사슴 : 얼음, 단단한녹용, 청녹용
	...
	ITEM_DROP_DATA[143] = [[-1], [0, 137, 3, 50]] # 복어장군 : 복어의심장
	...
	ITEM_DROP_DATA[150] = [[-1], [0, 138, 3, 50]] # 해마장군 : 해마의심장
	
	ITEM_DROP_DATA[151] = [[-1], [0, 139, 1, 20]] # 고양인어 : 인어의심장
	ITEM_DROP_DATA[152] = [[-1], [0, 139, 1, 20]] # 이쁜이인어 : 인어의심장
	ITEM_DROP_DATA[153] = [[-1], [0, 139, 3, 50]] # 인어장군 : 인어의심장
	
	ITEM_DROP_DATA[154] = [[-1], [0, 145, 1, 1], [0, 140, 1, 15]] # 외칼상어 : 상어의핵, 상어의심장
	ITEM_DROP_DATA[155] = [[-1], [0, 145, 1, 1], [0, 140, 1, 15]] # 쌍칼상어 : 상어의핵, 상어의심장
	ITEM_DROP_DATA[156] = [[-1], [0, 145, 1, 10], [0, 140, 3, 40]] # 상어장군 : 상어의핵, 상어의심장
	
	ITEM_DROP_DATA[157] = [[-1], [0, 142, 1, 10], [0, 141, 1, 4, 378]] # 해파리수하 : 해파리의심장, 전략문서
	ITEM_DROP_DATA[158] = [[-1], [0, 142, 3, 40]] # 해파리장군 : 해파리의심장
	...
	ITEM_DROP_DATA[178] = [[-1], [0, 29, 1, 30], [0, 30, 1, 30], [0, 148, 1, 1]] # 단선녀 : 호박, 진호박, 참나무조각
	ITEM_DROP_DATA[179] = [[-1], [0, 29, 1, 30], [0, 30, 1, 30], [0, 148, 1, 1]] # 파선녀 : 호박, 진호박, 참나무조각
	ITEM_DROP_DATA[180] = [[-1], [0, 162, 1, 30], [0, 148, 1, 5]] # 견귀 : 희귀진호박, 참나무조각
	
	ITEM_DROP_DATA[181] = [[-1], [0, 29, 1, 30], [0, 30, 1, 30], [0, 148, 1, 0.5]] # 맹오 : 호박, 진호박, 참나무조각
	ITEM_DROP_DATA[182] = [[-1], [0, 29, 1, 40], [0, 30, 1, 45], [0, 37, 1, 30]] # 문위 : 호박, 진호박, 불의결정
	ITEM_DROP_DATA[194] = [[-1], [0, 162, 1, 40], [0, 37, 1, 60], [0, 148, 1, 5]] # 문려 : 희귀진호박, 불의결정, 참나무조각
	
	ITEM_DROP_DATA[183] = [[-1], [0, 29, 1, 30], [0, 30, 1, 30], [0, 148, 1, 0.5]] # 욘주겐 : 호박, 진호박, 참나무조각
	ITEM_DROP_DATA[184] = [[-1], [0, 29, 1, 30], [0, 30, 1, 30], [0, 148, 1, 0.5]] # 바주겐 : 호박, 진호박, 참나무조각
	ITEM_DROP_DATA[185] = [[-1], [0, 29, 1, 30], [0, 30, 1, 30], [0, 148, 1, 0.5]] # 나주겐 : 호박, 진호박, 참나무조각
	ITEM_DROP_DATA[186] = [[-1], [0, 162, 1, 45], [0, 37, 2, 30], [0, 148, 1, 5]] # 무사 : 희귀진호박, 불의결정, 참나무조각
	
	ITEM_DROP_DATA[187] = [[-1], [0, 29, 1, 70], [0, 30, 1, 60], [0, 148, 1, 0.5]] # 선월 : 호박, 진호박, 참나무조각
	ITEM_DROP_DATA[188] = [[-1], [0, 29, 1, 70], [0, 30, 1, 60], [0, 148, 1, 0.5]] # 이광 : 호박, 진호박, 참나무조각
	ITEM_DROP_DATA[189] = [[-1], [0, 162, 2, 70], [0, 37, 2, 30], [0, 148, 1, 10]] # 주마관 : 희귀진호박, 불의결정, 참나무조각
	
	ITEM_DROP_DATA[190] = [[-1], [0, 153, 1, 1]] # 망령 : 도깨비가죽
	ITEM_DROP_DATA[191] = [[-1], [0, 155, 1, 60], [0, 37, 2, 30]] # 유성지 : 유성지의보패, 참나무조각
	ITEM_DROP_DATA[192] = [[-1], [0, 151, 1, 60], [0, 37, 2, 30]] # 해골왕 : 해골왕의뼈, 참나무조각
	...
	ITEM_DROP_DATA[231] = [[-1], [2, 55, 1, 1], [0, 169, 10, 100], [0, 167, 10, 100], [0, 175, 1, 60]] # 천구왕 : 망또4, 녹색, 갈색구슬조각, 동천패
	...
	# 폭염도 화산굴
	ITEM_DROP_DATA[250] = [[1], [0, 162, 1, 10], [0, 215, 1, 10]] # 철보장 : 희귀진호박, 금조각
	ITEM_DROP_DATA[251] = [[1], [0, 162, 1, 10], [0, 216, 1, 10]] # 철거인 : 희귀진호박, 반짝이는돌
