class MrMo_ABS
	...
	def filter_valid_drops(drop_data)
		...
		drop_data.each do |d|
			...
			chance = chance_multiplier * [$drop_event, 1].max * 10.0
	...
	ITEM_DROP_DATA[40] = [[-1], [0, 21, 1, 10], [3, 500, 1, 60]] # 자생원 : 산삼, 500전
	...
	ITEM_DROP_DATA[77] = [[-1], [0, 59, 1, 70], [1, 27, 1, 10]] # 청산숲돼지: 청산돼지뿔, 철검
	...
	ITEM_DROP_DATA[29] = [[-1], [0, 22, 3, 60], [3, 500, 1, 20], [1, 27, 1, 30]] # 적호 : 짙은호랑이고기, 500전, 철검
	...
	ITEM_DROP_DATA[66] = [[-1], [0, 29, 3, 80], [0, 30, 3, 80], [0, 116, 1, 5], [1, 108, 1, 1]] # 인형술사 : 호박, 진호박, 황금호박, 백화검
	...
	ITEM_DROP_DATA[41] = [[-1], [0, 38, 1, 10], [0, 39, 1, 1]] # 청자다람쥐 : 작은상자, 고급상자
	ITEM_DROP_DATA[49] = [[-1], [0, 38, 1, 100], [0, 39, 1, 60], [0, 92, 1, 10]] # 고래 :  작은상자, 고급상자, 최고급상자
	...
	ITEM_DROP_DATA[48] = [[-1], [0, 29, 1, 40], [0, 30, 1, 40], [0, 31, 1, 10]] # 도깨비불 : 호박, 진호박, 불의혼
	...
	ITEM_DROP_DATA[82] = [[-1], [0, 60, 2, 60], [2, 4, 1, 10]] # 구미호 : 쇠조각, 사각방패
	...
	ITEM_DROP_DATA[193] = [[-1], [2, 55, 1, 2], [0, 154, 1, 40], [0, 37, 3, 100]] # 파괴왕 : 망또4, 순수의강철, 참나무조각
	...
	# -------
	# 고균도
	ITEM_DROP_DATA[50] = [[-1], [0, 44, 1, 5]] # 녹웅객 : 낡은 수리검
	ITEM_DROP_DATA[57] = [[-1], [0, 44, 1, 5], [0, 51, 3, 10, 141]] # 청웅객 : 낡은 수리검, 청웅의환
	...
	# 난파선
	ITEM_DROP_DATA[243] = [[-1], [0, 209, 1, 5]] # 최가 : 기름자루
	ITEM_DROP_DATA[244] = [[-1], [0, 211, 1, 5]] # 백보 : 향료가루
	ITEM_DROP_DATA[245] = [[-1], [0, 209, 1, 5], [0, 211, 1, 5]] # 엽춘 : 기름자루, 향료가루
	ITEM_DROP_DATA[246] = [[-1], [0, 209, 1, 100], [0, 211, 1, 100], [0, 201, 1, 10], [0, 202, 1, 60], [0, 203, 1, 10], [0, 204, 1, 10], [0, 213, 1, 20]] # 선장망령 : 기름자루, 향료가루, 대지의토템, 바람의토템, 번개의토템, 화염의토템, 정령의결정
	
	# 가릉도 숲지대
	ITEM_DROP_DATA[247] = [[-1], [0, 161, 1, 2]] # 사마귀 : 희귀호박
	ITEM_DROP_DATA[248] = [[-1], [0, 161, 1, 2], [0, 214, 1, 10]] # 독화 : 희귀호박, 독니
	ITEM_DROP_DATA[249] = [[-1], [0, 201, 1, 10], [0, 202, 1, 10], [0, 203, 1, 60], [0, 204, 1, 10], [0, 214, [2, 5], 100], [0, 218, 1, 20]] # 야월진랑 : 대지의토템, 바람의토템, 번개의토템, 화염의토템, 독니, 무소의뿔
	
	# 폭염도 화산굴
	ITEM_DROP_DATA[250] = [[-1], [0, 162, 1, 1], [0, 215, 1, 5]] # 철보장 : 희귀진호박, 금조각
	ITEM_DROP_DATA[251] = [[-1], [0, 162, 1, 1], [0, 216, 1, 5]] # 철거인 : 희귀진호박, 반짝이는돌
	ITEM_DROP_DATA[252] = [[-1], [2, 56, 1, 1], [0, 215, 1, 100], [0, 216, 1, 100], [0, 201, 1, 10], [0, 202, 1, 10], [0, 203, 1, 10], [0, 204, 1, 60], [0, 206, 1, 15], [0, 207, 1, 15]] # 마려 : 망또5, 대지의토템, 바람의토템, 번개의토템, 화염의토템, 자연의인'상, 자연의인'하
	...
	ITEM_DROP_DATA[267] = [[1], [3, 222222, 1, 5]] # 부자다람쥐 : 222222전
	...
	ITEM_DROP_DATA[279] = [[1], [3, 333333, 1, 5]] # 돈돈돈 : 333333전
