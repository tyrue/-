class MrMo_ABS
	...
	ITEM_DROP_DATA[77] = [[-1], [0, 59, 1, 70], [1, 23, 1, 30]] # 청산숲돼지: 청산돼지뿔, 철도
	
	# 사슴굴
	ITEM_DROP_DATA[23] = [[-1], [0, 5, 3, 80]] # 주홍사슴 : 사슴고기
	ITEM_DROP_DATA[25] = [[-1], [0, 6, 5, 80], [1, 22, 1, 5]] # 흑/백순록 : 녹용, 비철단도
	ITEM_DROP_DATA[76] = [[2], [0, 6, 30, 60], [0, 29, 1, 40], [1, 22, 1, 20]] # 청순록 : 녹용, 호박, 비철단도
	...
	# 흉가
	ITEM_DROP_DATA[34] = [[1], [0, 29, 1, 40], [0, 30, 1, 40]] # 달걀귀신 : 호박, 진호박
	ITEM_DROP_DATA[35] = [[1], [0, 29, 1, 40], [0, 30, 1, 40]] # 몽달귀신 : 호박, 진호박
	ITEM_DROP_DATA[39] = [[-1], [0, 30, 1, 40], [0, 31, 1, 30], [0, 37, 1, 10]] # 불귀신 : 진호박, 불의혼, 불의결정
	ITEM_DROP_DATA[36] = [[-1], [0, 29, 3, 60], [0, 30, 3, 60], [0, 116, 1, 10]] # 처녀귀신 : 호박, 진호박, 황금호박
	...
	# 도깨비굴
	ITEM_DROP_DATA[47] = [[1], [0, 29, 1, 30], [0, 30, 1, 30]] # 도깨비 : 호박, 진호박
	ITEM_DROP_DATA[48] = [[1], [0, 29, 1, 30], [0, 30, 1, 30]] # 도깨비불 : 호박, 진호박
	ITEM_DROP_DATA[80] = [[1], [2, 33, 1, 15], [1, 129, 1, 15]] # 청명도깨비 : 도깨비부적, 도깨비방망이
	...
	ITEM_DROP_DATA[82] = [[-1], [0, 60, 2, 60]] # 구미호 : 쇠조각
	...
	ITEM_DROP_DATA[111] = [[3], [0, 70, 5, 70], [1, 125, 1, 30], [1, 129, 1, 15], [2, 44, 1, 5], [1, 130, 1, 5]] # 산적왕 : 빨간시약, 일월대도, 도깨비방망이, 황혼의활복, 산적왕의칼
	...
	ITEM_DROP_DATA[101] = [[-1], [0, 29, 3, 60], [0, 30, 3, 60], [0, 116, 1, 10]] # 일본세작대장 : 호박, 진호박, 황금호박
	...
	# 극지방
	ITEM_DROP_DATA[105] = [[-1], [0, 103, 20, 50], [0, 116, 1, 30], [0, 58, 1, 15]] # 에메랄드인형 : 얼음, 황금호박, 청녹용
	ITEM_DROP_DATA[109] = [[-1], [0, 103, 2, 40]] # 눈괴물 : 얼음
	ITEM_DROP_DATA[110] = [[-1], [0, 103, 2, 40], [0, 101, 2, 10], [0, 58, 1, 4]] # 북극사슴 : 얼음, 단단한녹용, 청녹용
	...
	ITEM_DROP_DATA[119] = [[2],  [0, 120, 1, 30], [0, 121, 1, 30], [0, 106, 1, 40]] # 백호왕 : 크리스탈, 수정, 건괘
	...
	ITEM_DROP_DATA[132] = [[-1], [0, 123, 1, 30], [0, 122, 1, 100]] # 건룡 : 건룡의어금니, 은나무가지
	ITEM_DROP_DATA[133] = [[-1], [0, 124, 1, 30], [0, 122, 1, 100]] # 감룡 : 감룡의어금니, 은나무가지
	ITEM_DROP_DATA[134] = [[-1], [0, 125, 1, 30], [0, 122, 1, 100]] # 진룡 : 진룡의어금니, 은나무가지
	
	# 용궁
	ITEM_DROP_DATA[141] = [[-1], [0, 137, 1, 20]] # 복돌 : 복어의심장
	ITEM_DROP_DATA[142] = [[-1], [0, 137, 1, 20]] # 복순 : 복어의심장
	ITEM_DROP_DATA[143] = [[-1], [0, 137, 3, 70]] # 복어장군 : 복어의심장
	...
	ITEM_DROP_DATA[150] = [[-1], [0, 138, 3, 70]] # 해마장군 : 해마의심장
	
	ITEM_DROP_DATA[151] = [[-1], [0, 139, 1, 20]] # 고양인어 : 인어의심장
	ITEM_DROP_DATA[152] = [[-1], [0, 139, 1, 20]] # 이쁜이인어 : 인어의심장
	ITEM_DROP_DATA[153] = [[-1], [0, 139, 3, 70]] # 인어장군 : 인어의심장
	
	ITEM_DROP_DATA[154] = [[-1], [0, 145, 1, 4], [0, 140, 1, 15]] # 외칼상어 : 상어의핵, 상어의심장
	ITEM_DROP_DATA[155] = [[-1], [0, 145, 1, 4], [0, 140, 1, 15]] # 쌍칼상어 : 상어의핵, 상어의심장
	...
	ITEM_DROP_DATA[159] = [[3], [2, 55, 1, 1], [0, 161, 1, 60], [0, 162, 1, 60], [0, 120, 1, 30], [0, 121, 1, 30], [1, 112, 1, 40]] # 거북장군 : 망또4, 희귀호박, 희귀진호박, 수정, 크리스탈, 양첨목봉
	...
	ITEM_DROP_DATA[176] = [[-1], [0, 161, 1, 35], [0, 148, 1, 5]] # 백향 : 희귀호박, 참나무조각
	
	ITEM_DROP_DATA[177] = [[-1], [0, 152, 1, 10]] # 하선녀 : 하선녀의실타래
	...
	ITEM_DROP_DATA[180] = [[-1], [0, 162, 1, 30], [0, 148, 1, 5]] # 견귀 : 희귀진호박, 참나무조각
	
	ITEM_DROP_DATA[181] = [[-1], [0, 29, 1, 30], [0, 30, 1, 30], [0, 148, 1, 0.7]] # 맹오 : 호박, 진호박, 참나무조각
	ITEM_DROP_DATA[182] = [[-1], [0, 29, 1, 40], [0, 30, 1, 45], [0, 37, 1, 13], [0, 148, 1, 0.7]] # 문위 : 호박, 진호박, 불의결정, 참나무조각
	ITEM_DROP_DATA[194] = [[-1], [0, 162, 1, 40], [0, 37, 1, 60], [0, 148, 1, 5]] # 문려 : 희귀진호박, 불의결정, 참나무조각
	
	ITEM_DROP_DATA[183] = [[-1], [0, 29, 1, 30], [0, 30, 1, 30], [0, 148, 1, 1]] # 욘주겐 : 호박, 진호박, 참나무조각
	ITEM_DROP_DATA[184] = [[-1], [0, 29, 1, 30], [0, 30, 1, 30], [0, 148, 1, 1]] # 바주겐 : 호박, 진호박, 참나무조각
	ITEM_DROP_DATA[185] = [[-1], [0, 29, 1, 30], [0, 30, 1, 30], [0, 148, 1, 1]] # 나주겐 : 호박, 진호박, 참나무조각
	ITEM_DROP_DATA[186] = [[-1], [0, 162, 1, 45], [0, 37, 2, 30], [0, 148, 1, 10]] # 무사 : 희귀진호박, 불의결정, 참나무조각
	
	ITEM_DROP_DATA[187] = [[-1], [0, 29, 1, 40], [0, 30, 1, 40], [0, 148, 1, 1]] # 선월 : 호박, 진호박, 참나무조각
	ITEM_DROP_DATA[188] = [[-1], [0, 29, 1, 40], [0, 30, 1, 40], [0, 148, 1, 1]] # 이광 : 호박, 진호박, 참나무조각
	ITEM_DROP_DATA[189] = [[-1], [0, 162, 2, 70], [0, 37, 2, 30], [0, 148, 1, 15]] # 주마관 : 희귀진호박, 불의결정, 참나무조각
	...
	ITEM_DROP_DATA[193] = [[-1], [0, 154, 1, 100], [0, 37, 3, 30]] # 파괴왕 : 순수의강철, 참나무조각
	...
	ITEM_DROP_DATA[204] = [[-1], [0, 169, 2, 100], [0, 167, 2, 100], [3, 30000, 1, 20]] # 인묘 : 녹색구슬조각, 갈색구슬조각
	
	ITEM_DROP_DATA[205] = [[-1], [0, 170, 1, 30]] # 청염유 : 청색구슬조각
	ITEM_DROP_DATA[206] = [[-1], [0, 168, 1, 30]] # 자염유 : 자색구슬조각 
	ITEM_DROP_DATA[207] = [[1], [0, 169, 1, 60], [0, 167, 1, 60]] # 염유장군 : 녹색구슬조각, 갈색구슬조각 
	ITEM_DROP_DATA[208] = [[-1], [0, 171, 1, 30], [0, 169, 2, 100], [0, 167, 2, 100]] # 염유왕 : 갈색보주, 녹색,갈색 구슬조각
	
	ITEM_DROP_DATA[209] = [[-1], [0, 170, 1, 30]] # 청기린 : 청색구슬조각
	ITEM_DROP_DATA[210] = [[-1], [0, 168, 1, 30]] # 자기린 : 자색구슬조각
	ITEM_DROP_DATA[211] = [[1], [0, 169, 1, 60], [0, 167, 1, 60]] # 흑마기린 : 녹색구슬조각, 갈색구슬조각
	ITEM_DROP_DATA[212] = [[-1], [0, 172, 1, 30], [0, 169, 2, 100], [0, 167, 2, 100]] # 기린왕 : 자색보주
	
	ITEM_DROP_DATA[213] = [[-1], [0, 190, 1, 1], [0, 170, 1, 30]] # 청악어 : 악어의피, 청색구슬조각
	ITEM_DROP_DATA[214] = [[-1], [0, 190, 1, 1], [0, 168, 1, 30]] # 자악어 : 악어의피, 자색구슬조각
	ITEM_DROP_DATA[215] = [[-1], [0, 173, 1, 30], [0, 169, 2, 100], [0, 167, 2, 100]] # 악어왕 : 녹색보주
	...
	ITEM_DROP_DATA[220] = [[-1], [0, 176, 1, 50], [0, 169, 3, 100], [0, 167, 3, 100]] # 산소괴왕 : 동지패
	...
	ITEM_DROP_DATA[224] = [[-1], [0, 174, 1, 30], [0, 169, 3, 100], [0, 167, 3, 100]] # 괴성왕 : 청색보주
	...
	ITEM_DROP_DATA[228] = [[-1], [0, 177, 1, 50], [0, 169, 4, 100], [0, 167, 4, 100]] # 뇌신왕 : 동인패
	...
	# 도삭산
	ITEM_DROP_DATA[232] = [[-1], [2, 56, 1, 1], [0, 194, 3, 100], [0, 195, 1, 30], [3, 1000000, 1, 0.1], [1, 112, 1, 10]] # 산신대왕 : 망또5, 산신의바늘, 산신의천, 돈, 양첨목봉
	ITEM_DROP_DATA[233] = [[2], [0, 194, 1, 1], [0, 162, 1, 10], [1, 26, 1, 1], [2, 47, 1, 0.1]] # 산신전사 : 산신의바늘, 희귀진호박, 녹호박별검, 망또1
	ITEM_DROP_DATA[234] = [[2], [0, 194, 1, 1], [0, 162, 1, 10], [1, 137, 1, 10], [2, 48, 1, 0.1]] # 산신도사 : 산신의바늘, 희귀진호박, 영혼죽장, 망또2
	ITEM_DROP_DATA[235] = [[2], [0, 194, 1, 1], [0, 162, 1, 10], [1, 130, 1, 1], [2, 49, 1, 0.1]] # 산신도적 : 산신의바늘, 희귀진호박, 산적왕의칼, 망또3
	ITEM_DROP_DATA[236] = [[2], [0, 194, 1, 1], [0, 162, 1, 10], [1, 107, 1, 10], [2, 47, 1, 0.1]] # 산신주술사 : 산신의바늘, 희귀진호박, 불의영혼봉, 망또1
	...
	ITEM_DROP_DATA[259] = [[-1], [0, 162, 2, 10], [0, 215, 2, 10], [0, 216, 1, 10], [0, 220, 2, 100], [0, 98, 1, 3], [3, 1000000, 1, 0.1]] # 가릉빈가 : 희귀진호박, 금조각, 반짝이는돌, 가릉빈가의 날개, 삿갓머리 화서
	...
	ITEM_DROP_DATA[266] = [[1], [0, 81, 1, 40]] # 날쎈다람쥐 : 속도시약
