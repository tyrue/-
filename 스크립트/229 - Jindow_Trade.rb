...
class Jindow_Trade < Jindow
	def initialize
		...
		@player_money = J::Type.new(self).refresh(@equip_1[@equip_1.size - 1].x + 30, @equip_1[@equip_1.size - 1].y + 30, 60, 18)
		@money = J::Button.new(self).refresh(40, "입력")
		@money.x = @player_money.x
		@money.y = @player_money.y + 30
	...
	def update
		super
		@trade_money = @player_money.result # 내가 올린 돈
		...
		for i in 1..$MAX_TRADE
			if $item_number[i] != nil and $item_number[i].is_ok != true
				...
				@item_txt[i].bitmap = Bitmap.new(240, 200)
		...
		if @a.click
			...
			if @trade_money == nil or @trade_money == ""
				$console.write_line("금전을 입력하세요.")
